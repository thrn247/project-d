...
- **Derived via `useMemo`:**
  ...
  - `metrics` — 3 KPIs: cohort count, avg admission risk %, **readmit rate among admitted = count(Stage_2 ≥ thresholds.readmission) / count(Predicted_Admission == 1)**. `thresholds` is a prop owned by [App.jsx](diabetesDashboard/src/App.jsx) that fetches `/data/thresholds.json` once and passes it down.
