...
# CELL 7: Evaluation
c7_source = """from sklearn.metrics import classification_report, precision_recall_curve, roc_auc_score, auc
import numpy as np
...
# CELL 8: Confusion Matrices (Update slightly if needed, but mostly same)
c8_source = """import matplotlib.pyplot as plt
...
# CELL 9: ROC and PR Curves (Massive upgrade)
c9_source = """from sklearn.metrics import roc_curve, precision_recall_curve, auc
...
cells = nb.get('cells', [])
cells[4]['source'] = format_source(c4_source)
cells[5]['source'] = format_source(c5_source)
cells[6]['source'] = format_source(c6_source)
cells[7]['source'] = format_source(c7_source)
cells[8]['source'] = format_source(c8_source)
cells[9]['source'] = format_source(c9_source)

# Clear outputs for these cells so the user has to run them to see new results
for i in range(4, 10):
    cells[i]['outputs'] = []
