...
//
// `getTips` is a factory so the threshold values inside detail strings stay in
// sync with the live `/data/thresholds.json` instead of hardcoded literals.

export const getTips = (thresholds) => {
  const adm = (thresholds.admission * 100).toFixed(1);
  ...
  return {
    ...
    avg_admission_risk: {
      ...
      detail: `A patient is flagged for admission when their individual score reaches the threshold of ${adm}% (XGBoost, F1-optimised on training data).`,
    },
    readmit_conversion: {
      ...
      detail: `Readmission threshold: ${readm}% (XGBoost, F1-optimised). Computed only across patients already flagged for admission.`,
    },
    age_band_chart: {
    ...
    risk_distribution: {
      ...
      detail: `Admission threshold: ${adm}%.`,
    },
    severity_sex_matrix: {
